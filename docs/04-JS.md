...
本地 JS 文件物理存储目录位于 **./script/JSFile**，在 RULS/REWRITE/TASK/WEBHOOK 中调用时，直接使用对应的文件名即可。支持二级目录，比如在 JSFile 下再建一个文件夹 test，放置一个文件 exam.js，在引用时使用 test/exam.js 即可
...
// 测试 JS: console.log(__dirname, __version, __home, __efss)
```
