...
支持多级目录，比如在 JSFile 下有文件夹 test，放置文件 exam.js，在引用时对应的文件名为 test/exam.js。
...
- __taskname       // 启动该脚本的任务名称 (v3.4.4 仅在从定时任务模块运行脚本时有值，其他时候为 undefined)
- __taskid         // 启动该脚本的任务  id (v3.4.4 仅在从定时任务模块运行脚本时有值，其他时候为 undefined)
...
- $task  ;定时任务管理（v3.4.4）
- $fend.clear()    ;清空 efh 文件运行产生的缓存（v3.5.5）
- 其他功能待添加
...
- 在使用定时任务或 webhook 等方式运行脚本时，可自定义添加临时环境变量（具体参考 [06-task.md](https://github.com/elecV2/elecV2P-dei/blob/master/docs/06-task.md) 运行 JS 相关部分）
...
该变量/函数仅在开启 sudo 模式的脚本中有效。（sudo 模式开启方式: 在脚本中添加 **// @grant  sudo**）
...
## $fend - favend 前后端数据交互(v3.5.5)

$fend 函数目前仅适用于 favend 的 efh 文件中。基本用法： $fend(key, data);
