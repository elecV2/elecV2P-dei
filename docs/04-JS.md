...
物理存储目录位于 **./script/JSFile**，在 RULS/REWRITE/TASK/WEBHOOK 中调用时，直接使用对应的文件名即可。

本地文件可在 webUI -> JSMANAGE 中查看和修改。
...
#### 特殊变量
``` 
- $results         // JS 最终返回结果。可以是 object/string,甚至是一个 promise 函数
- require          // 直接引用其他 nodejs 模块
```

JS 文件开头使用 **// @grant  require** 强制在脚本中使用 require 函数
**// @grant  surge** 强制开启 surge 脚本调试模式
**// @grant  quanx** 强制开启 quanx 脚本调试模式

*如果没有 @grant 声明，脚本则会自动进行判断。*
